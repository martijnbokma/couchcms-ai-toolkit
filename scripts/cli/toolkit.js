...
function executeScript(scriptName, projectDir, extraArgs = []) {
    const scriptPath = join(TOOLKIT_ROOT, 'scripts', 'cli', scriptName)
...
async function handleServe(projectDir, options) {
    ...
    const noWatch = options.noWatch || false

    let watchProcess = null

    // Start watch mode in development (unless --no-watch is specified)
    if (!noWatch) {
        ...
        try {
            ...
            watchProcess.on('error', (error) => {
                printError(`Failed to start watch mode: ${error.message}`)
                printInfo('Continuing without watch mode...')
            })

            // Give watch mode a moment to start and do initial build
            await new Promise(resolve => setTimeout(resolve, 2000))
            printSuccess('Watch mode started!')
        } catch (error) {
            printError(`Failed to start watch mode: ${error.message}`)
            printInfo('Continuing without watch mode...')
        }
    } else {
        // Build once if watch is disabled
        ...
        try {
            ...
            if (cleanResult.status !== 0) {
                printError('Failed to clean build artifacts')
                process.exit(1)
            }

            // Run build
            const buildResult = spawnSync('bun', [buildScript], {
                cwd: TOOLKIT_ROOT,
                stdio: 'inherit'
            })

            if (buildResult.status !== 0) {
                printError('Failed to build web assets')
                process.exit(1)
            }

            printSuccess('Build complete!')
        } catch (error) {
            printError(`Failed to build web assets: ${error.message}`)
            process.exit(1)
        }
    }
...
Options:
  ...
  --no-watch              Disable watch mode for serve (build once instead)
...
Examples:
  ...
  toolkit serve                      # Start browser setup interface (with watch mode)
  toolkit serve --port=3000          # Start on specific port
  toolkit serve --no-watch           # Start without watch mode (build once)
