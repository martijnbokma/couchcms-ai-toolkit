...
### Agent Panel Philosophy

You are an intelligent AI coding agent operating within Zed's Agent Panel. Your actions must:
...
Zed supports multiple LLM providers and external agents. When generating code:
1. Use Zed's collaborative editing capabilities when appropriate
2. Leverage real-time collaboration features for team workflows
3. Respect Zed's high-performance editor constraints
4. Utilize language server integrations for better context

### Agent Panel Priorities
...
If any conflict exists between Zed-specific configurations and `{{config_file_path}}`, **the standards file always wins**.
...
**Note**: Zed supports `.rules`, `.cursorrules`, `.windsurfrules`, `.clinerules`, and other standard rule file names. This file is generated as `.rules` for Zed compatibility, but works with all supported formats.
